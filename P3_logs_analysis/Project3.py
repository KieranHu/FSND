...
query2 = "SELECT authors.name, count(*) as number FROM articles, " \
         "authors, log WHERE articles.author = authors.id " \
         "AND log.path LIKE ('%'||articles.slug||'%') " \
         "AND log.status = '200 OK' GROUP BY authors.name " \
...
query3 = "SELECT time, error FROM (" \
         "SELECT a.time, " \
         "round((b.errortotal::float/a.alltotal::float)::numeric, 4) " \
         "AS error " \
         "FROM " \
